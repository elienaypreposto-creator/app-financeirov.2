...
function ConciliacaoView({ onImportar, onEditGroup, userId }: { onImportar: () => void, onEditGroup: (group: any) => void, userId: string | null }) {
  ...
  const loadData = async () => {
    ...
    if (txs && !error) {
      ...
      const processed = Object.values(groupsMap).map(g => {
        ...
        // Formatar período robusto com dia/mês/ano completo
        if (g.minDate && g.maxDate) {
           const parseF = (s: string) => {
              ...
              if (s.includes('-')) {
                 const pts = s.split('-');
                 if (pts[0].length === 4) return `${pts[2]}/${pts[1]}/${pts[0]}`; // YYYY-MM-DD -> DD/MM/YYYY
                 return `${pts[0]}/${pts[1]}/${pts[2]}`; // DD-MM-YYYY -> DD/MM/YYYY
              } else if (s.includes('/')) {
                 const pts = s.split('/');
                 if (pts[0].length === 4) return `${pts[2]}/${pts[1]}/${pts[0]}`; // YYYY/MM/DD -> DD/MM/YYYY
                 return `${pts[0]}/${pts[1]}/${pts[2]}`; // DD/MM/YYYY
              }
              return s;
           };
  ...
  const handleDeleteGroup = async (group: any) => {
    ...
    // Atualização otimista: remove visualmente antes de aguardar o banco
      ...
      <div className="border border-slate-100 rounded-2xl overflow-hidden mt-4 bg-white shadow-xl">
        <table className="w-full text-sm text-left text-slate-700">
          <thead className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 border-b border-slate-100">
            <tr>
              ...
              <th className="px-6 py-5 border-x-2 border-slate-100 bg-slate-50 min-w-[120px] text-center">Período</th>
              ...
              <tr key={group.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <div className="flex justify-center gap-4 text-slate-400">
                     <span title="Continuar Classificando" className="bg-emerald-50 text-emerald-600 p-2 rounded-lg cursor-pointer hover:bg-emerald-500 hover:text-white transition-all border border-emerald-200" onClick={() => onEditGroup(group)}><PencilLine className="w-4 h-4" /></span>
                     <span title="Excluir Extrato Inteiro" className="bg-rose-50 text-rose-500 p-2 rounded-lg cursor-pointer hover:bg-rose-500 hover:text-white transition-all border border-rose-200" onClick={() => handleDeleteGroup(group)}><Trash className="w-4 h-4" /></span>
                  </div>
                </td>
              </tr>
            ))}
        ...
        {!isLoading && filteredGroups.length === 0 && (
           <div className="flex items-center justify-center p-12 mb-2">
             <div className="bg-slate-50 border border-slate-200 rounded-xl px-8 py-5 text-slate-400 text-sm font-medium">
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl w-full mx-auto pb-20">
      ...
      <Dialog open={isNewCategoryModalOpen} onOpenChange={setNewCategoryModalOpen}>
        <DialogContent className="sm:max-w-md bg-white border-slate-200 text-slate-800 shadow-2xl">
          ...
          <div className="space-y-4 pt-4">
             <div className="space-y-1.5">
               ...
               <input type="text" value={newCatName} onChange={e=>setNewCatName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm text-slate-800 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition-all" placeholder="Ex: Combustível" />
             </div>
      ...
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={handleIntentBack} className="text-slate-500 hover:text-emerald-600 bg-white hover:bg-emerald-50 border border-slate-200 rounded-xl shadow-sm">
           <ArrowLeft className="w-5 h-5" />
        </Button>
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tighter">Configuração de Ficheiro</h1>
          <p className="text-slate-400 text-xs font-bold mt-1 tracking-wide">Realize a leitura inteligente do extrato bancário</p>
        </div>
      </div>

      <Card className="bg-white border-slate-200 shadow-xl overflow-visible relative rounded-2xl border-b-4 border-b-emerald-500">
        ...
        <CardContent className="pt-6 pb-6 flex flex-col md:flex-row items-center justify-between gap-6 z-10 relative">
           <div className="flex items-center gap-4 cursor-pointer hover:opacity-80 transition-opacity flex-1 bg-slate-50 p-4 border border-dashed border-emerald-400/40 rounded-xl" onClick={handleFileUploadClick}>
           ...
           {!loadingAccounts && registeredAccounts.length === 0 && (
             <div className="absolute inset-0 bg-white/95 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-6 text-center rounded-xl border-2 border-dashed border-rose-400/50">
                ...
                <Button onClick={onBack} variant="outline" className="border-slate-200 text-slate-600 hover:bg-slate-50">Voltar</Button>
             </div>
           )}
           ...
           <div className="flex gap-4 items-center">
             <div className="flex flex-col gap-1.5">
               ...
               <Select value={selectedBank} onValueChange={(v: string | null) => {
                 ...
                 <SelectTrigger className="w-[200px] bg-white border-slate-200 text-slate-700 h-10 font-bold rounded-xl focus:ring-emerald-500">
             ...
             <div className="flex flex-col gap-1.5">
               <span className="text-xs font-bold text-slate-400 uppercase tracking-widest pl-1">Identificação</span>
                <div className="h-10 px-4 flex items-center justify-center bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-xl font-black text-sm tracking-widest min-w-[70px]">
              ...
              <table className="w-full text-sm text-left text-slate-700">
               ...
               <tbody>
                 {transactions.map((t) => {
                   ...
                   return (
                   <tr key={t.id} className={`border-b border-slate-50 transition-colors ${t.ignored ? 'opacity-30 grayscale bg-slate-100' : 'hover:bg-slate-50/50'} ${isChecked && !t.ignored ? 'bg-emerald-50/30' : ''}`}>
                     ...
                     <td className={`px-4 py-3 font-bold whitespace-nowrap text-right ${t.value > 0 ? "text-emerald-600" : "text-rose-500"}`}>
                        {formatCurrency(t.value)}
                     </td>
                     <td className="px-4 py-3">
                        <Select value={t.cat || ""} onValueChange={(val) => handleCategoryChange(val, t.id)} disabled={isChecked && !t.ignored}>
                          <SelectTrigger className={`w-[220px] bg-white border-slate-200 font-semibold shadow-sm transition-colors rounded-xl ${t.value > 0 ? 'text-emerald-600' : 'text-rose-500'} disabled:opacity-100 disabled:select-none`}>
                            <SelectValue placeholder="Selecionar Categoria" />
                          </SelectTrigger>
                          <SelectContent className="max-h-[300px] bg-white border-slate-200 rounded-xl">
                            {t.value >= 0 ? (
                               <SelectGroup>
                                 <SelectLabel className="text-emerald-500 tracking-widest uppercase text-[10px]">Entradas</SelectLabel>
                                 {categorias.entradas.map(c => <SelectItem key={c} value={c} className="font-semibold text-slate-700">{c}</SelectItem>)}
                               </SelectGroup>
                            ) : (
                               <SelectGroup>
                                 <SelectLabel className="text-rose-500 tracking-widest uppercase text-[10px]">Saídas</SelectLabel>
